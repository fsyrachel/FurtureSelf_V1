...
async def retrieve_rag_memory_async(
    ...
    try:
        # 1.必须 `await aembed_query`
        query_embedding = await embeddings_model.aembed_query(query)
        
        # 2. RAG 隔离查询 (1024 维)
        # SQLAlchemy *不支持* 异步 DB 查询 (除非用 asyncpg)
        # DB 查询 *仍然* 是 Sync，但 Embedding 是 Async
